...
:mod:`ED_INPUT_VARS`, global ones and shared classes/data structure in
:mod:`ED_VARS_GLOBAL`, global memory (de)allocation :mod:`ED_SETUP` as well as
generic functions which are used throughout the code :mod:`ED_AUX_FUNX`.
...
Some simple data structures contained in :mod:`ED_VARS_GLOBAL` have
been discussed in :doc:`structure/general/ed_vars_global`, such as  :code:`GFmatrix` type storing
all weights and poles of the Green's functions or
:code:`effective_bath` gathering the different bath components
according to value of :code:`bath_type` and :code:`ed_mode`.
However other tasks in the code requires more elaborated classes which
are implemented in two main modules.

The first is :mod:`ED_SPARSE_MATRIX` describing Compact Row Stored
...
In :mod:`ED_SECTOR` we implement the construction of the symmetry
...
selected using the variable :code:`bath_type=normal, hybrid, replica,
general` according to the nature of the problem at hand.
...
different cases corresponding to the value of :code:`bath_type`. 


For :code:`bath_type=normal` a number :code:`Nbath` of electronic
...
the orbital index.
For :code:`ed_mode=superc` the bath includes a set of parameters
:math:`\Delta_p` describing the superconductive amplitude on each bath
level. 


For :code:`bath_type=hybrid` a number :code:`Nbath` of electronic
...
For :code:`bath_type=general` a number :code:`Nbath` of copies of the
impurity structure are  coupled to the impurity itself.  Each bath
element is made of a number :math:`N_{orb}` of electronic levels,
...
.. note::
   ...
   upon calling :code:`get_bath_dimension`, see
   :doc:`structure/bath/ed_bath_dim`.


.. note::
   The :code:`replica, general` bath topologies are available also for
   the superconductive case :code:`ed_mode=superc`. In this case the
...
user a generic function :code:`ed_chi2_fitgf` performing the
