MODULE ED_OBSERVABLES_SUPERC
  ...
  !QUI ADDED SAMUELE parallel and antiparallel
  complex(8),dimension(:,:),allocatable :: vvinit_p
  complex(8),dimension(:,:),allocatable :: vvinit_a
  complex(8),dimension(:),  allocatable :: chi_sc_p
  complex(8),dimension(:),  allocatable :: chi_sc_a
...
contains 
  ...
  subroutine observables_superc()
    ...
    !QUI ADDED SAMUELE
    allocate(chi_sc_p(2));chi_sc_p=zero  
    allocate(chi_sc_a(2));chi_sc_a=zero 
    !
    Egs     = state_list%emin
    dens    = 0.d0
    dens_up = 0.d0
    ...
    do istate=1,state_list%size
       ...
       if(Mpimaster)then
          ...
          !QUI ADDED SAMUELE
          !EXCITONIC SUPEROCONDUCTIVE ORDER PARAMETER
          if(Nspin==1 .and. Norb==2)then
             ! A_{+/- , i,1} = CDGe_UP {+/-}{i,1} Ch_DW
             ! Chi_{+/-,i/1} = < ( ADG_{+/- , i,1} ) ( A_{+/- , i,1} ) > chi_sc(1/2)
             ! Chi_{+,1} + i*Chi_{-,i} = (1+i)(Nh_DW +1 -Ne_UP) + 2*Phi_eh
             !Phi_eh = < CDGe_UP CDGh_DW >         
             isz = getsz(isector)
             if(isz<Ns)then
                jsector = getsector(isz+1,1)
                call build_sector(jsector,sectorJ)
                allocate(vvinit_p(2,sectorJ%Dim));vvinit_p=zero
                allocate(vvinit_a(2,sectorJ%Dim));vvinit_a=zero
                do i=1,sectorI%Dim
                   call apply_op_CDG(i,j,sgn,1,1,1,sectorI,sectorJ)!c^+_e,up
                   if(sgn==0d0.OR.j==0)cycle
                   vvinit_p(1,j) = sgn*state_cvec(i)
                   vvinit_p(2,j) = sgn*state_cvec(i)
                   vvinit_a(1,j) = vvinit_p(1,j)
                   vvinit_a(2,j) = vvinit_p(2,j)
                enddo
                do i=1,sectorI%Dim
                   call apply_op_C(i,j,sgn,2,1,1,sectorI,sectorJ) !c_h,dw
                   if(sgn==0d0.OR.j==0)cycle
                   vvinit_p(1,j) = vvinit_p(1,j) + sgn*state_cvec(i)
                   vvinit_p(2,j) = vvinit_p(2,j) - xi*sgn*state_cvec(i)
                enddo
                do i=1,sectorI%Dim
                   call apply_op_C(i,j,sgn,2,1,2,sectorI,sectorJ) !c_h,up
                   if(sgn==0d0.OR.j==0)cycle
                   vvinit_a(1,j) = vvinit_a(1,j) + sgn*state_cvec(i)
                   vvinit_a(2,j) = vvinit_a(2,j) - xi*sgn*state_cvec(i)
                enddo
                call delete_sector(sectorJ)
                do i=1,Norb
                   chi_sc_p(i) = chi_sc_p(i) + dot_product(vvinit_p(i,:),vvinit_p(i,:))*peso
                   chi_sc_a(i) = chi_sc_a(i) + dot_product(vvinit_a(i,:),vvinit_a(i,:))*peso
                enddo
             endif
             if(allocated(vvinit_p))deallocate(vvinit_p)
             if(allocated(vvinit_a))deallocate(vvinit_a)
          endif
    ...
    enddo
    !QUI ADDED SAMUELE
    print*, "Phi exc parallel      :", 0.5d0*( chi_sc_p(1)+xi*chi_sc_p(2)-(1.d0+xi)*(dens_dw(2)+1-dens_up(1)) )
    print*, "Phi exc anti-parallel :", 0.5d0*( chi_sc_a(1)+xi*chi_sc_a(2)-(1.d0+xi)*(dens_up(2)+1-dens_up(1)) )
    print*, "Don't be lazy and make the code better!"
    if(allocated(chi_sc_a))deallocate(chi_sc_a)
    if(allocated(chi_sc_p))deallocate(chi_sc_p)
    !
    !
    if(MPIMASTER)then
       call get_szr
