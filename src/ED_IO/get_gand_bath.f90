subroutine ed_get_g0and(x,bath_,G0and,axis,type)
  ...
  L=size(x)
  select rank(g0and)
  rank default; stop "ed_get_g0and ERROR: g0and has a wrong rank"
  rank (3)
...
end subroutine ed_get_g0and



subroutine ed_get_delta(x,bath_,delta,axis,type)
  complex(8),dimension(:),intent(in)                  :: x
  real(8),dimension(:)                                :: bath_
  ...
  L=size(x)
  select rank(delta)
  rank default; stop "ed_get_delta ERROR: delta has a wrong rank"
  rank (3)
