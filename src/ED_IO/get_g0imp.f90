subroutine ed_get_g0imp_site(self,bath,axis,type)
  ...
  !
  select rank(self)
  rank default; stop "ed_get_g0imp ERROR: self has a wrong rank"
  rank (3)
...
end subroutine ed_get_g0imp_site



subroutine ed_get_g0imp_lattice(self,bath,axis,type)
  complex(8),dimension(..),intent(inout)        :: self
  real(8),dimension(:,:)                        :: bath
  character(len=*),optional                     :: axis
  character(len=*),optional                     :: type
...
end subroutine ed_get_g0imp_lattice
